...
@section('content')
        ...
        <tbody>
            @foreach($pet->weightRecords as $record)
                <tr>
                    ...
                    <td>
                        <form method="post" action="{{ route('weightrecords.destroy', $record) }}">
                            @csrf
                            @method('DELETE')
    ...
    <a class="inline-block bg-blue-600 text-white p-2 mt-4 self-start" href="{{ route('weightrecords.create', $pet) }}">
        Ajouter un poids
    </a>
@endsection
